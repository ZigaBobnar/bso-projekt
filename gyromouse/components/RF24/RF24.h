...
private:
  ...
  uint8_t spi_rxbuff[32+1] __attribute__ ((aligned (__BIGGEST_ALIGNMENT__))); //SPI receive buffer (payload max 32 bytes)
  uint8_t spi_txbuff[32+1] __attribute__ ((aligned (__BIGGEST_ALIGNMENT__))); //SPI transmit buffer (payload max 32 bytes + 1 byte for the command)
//  uint32_t spi_rxbuff[40];
//  uint32_t spi_txbuff[40];
