...
#include "RF24.h"
  ...
  #define delay(var) vTaskDelay(var / portTICK_PERIOD_MS)
  #define LOW 0
  #define HIGH 1
#endif
...
uint32_t millis()
{
  uint32_t tim = (TIMER(0).COUNT) ^ 0x7fffff;
  tim /= 1000;
  return(tim);
}
...
#if !defined (RF24_LINUX) && !defined (FreeRTOS)
	digitalWrite(csn_pin,mode);
...
/****************************************************************************/
  ...
  inline void RF24::endTransaction() {
    ...
    #if defined (RF24_SPI_TRANSACTIONS) && !defined (FreeRTOS)
       _SPI.endTransaction();
    #endif
  }
...
uint8_t RF24::read_register(uint8_t reg, uint8_t* buf, uint8_t len)
{
...
#if defined (FreeRTOS)
  beginTransaction();
  uint8_t * prx = (uint8_t *) spi_rxbuff;
  uint8_t * ptx = (uint8_t *) spi_txbuff;
  ...
  *ptx++ = ( R_REGISTER | ( REGISTER_MASK & reg ) );
  ...
  spi_transfer(1, (const void *) spi_txbuff, (void *) spi_rxbuff, (size_t) size, SPI_8BIT);
...
uint8_t RF24::spi_test(uint8_t reg)
{
//  uint8_t spi_rxbuff[10];
//  uint8_t spi_txbuff[10];
  uint8_t * prx = (uint8_t *) spi_rxbuff;
  uint8_t * ptx = (uint8_t *) spi_txbuff;
...
//  printf("command %x %x\n", spi_txbuff[0], spi_txbuff[1]);

  beginTransaction();
  spi_transfer(1, (const void *) spi_txbuff, (void *) spi_rxbuff, 2, SPI_8BIT);
  endTransaction();

//  printf("response %x %x %x %x txlen %i\n", spi_rxbuff[0], spi_rxbuff[1], spi_rxbuff[2], spi_rxbuff[3], txlen);

  return (*++prx);   // result is 2nd byte of receive buffer
}
...
uint8_t RF24::read_register(uint8_t reg)
{
  ...
  uint8_t * ptx = (uint8_t *) spi_txbuff;
  *ptx++ = ( R_REGISTER | ( REGISTER_MASK & reg ) );
  *ptx++ = 0x01 ; // Dummy operation, just for reading
...
uint8_t RF24::write_register(uint8_t reg, const uint8_t* buf, uint8_t len)
{
  ...
  *ptx++ = ( W_REGISTER | ( REGISTER_MASK & reg ) );
  ...
  beginTransaction();
  spi_transfer(1, (const void *) spi_txbuff, (void *) spi_rxbuff, size, SPI_8BIT);
...
//   uint8_t* current = reinterpret_cast<const uint8_t*>(buf); // Niko
...
//printf("AW %i  size of buff %i\n", addr_width, sizeof buffer);
...
//  _SPI.transfernb( (char *) spi_txbuff, (char *) spi_rxbuff, 2);
  ...
  if(result > 32) { flush_rx(); delay(10); return 0; } // delay(2)
  return result;
}
...
void RF24::writeAckPayload(uint8_t pipe, const void* buf, uint8_t len)
{
  ...
  uint8_t data_len = rf24_min(len,32);
    ...
    *ptx++ =  W_ACK_PAYLOAD | ( pipe & 0x07 );
    ...
    spi_transfer(1, (const void *) spi_txbuff, NULL, size, SPI_8BIT);
    endTransaction();
