...
void task_watchdog(void *pvParameters) {
	...
	while (true) {
		WRITE_COMMAND("watchdog", "alive(diff=%f)", ((float)current_time - gyromouse.timings.last_watchdog_check) * portTICK_PERIOD_MS);
		gyromouse.timings.last_watchdog_check = current_time;

		vTaskDelayUntil(&current_time, pdMS_TO_TICKS(5000));
	}
}
