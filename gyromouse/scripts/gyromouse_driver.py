...
class GyroMouseSerial:
    ...
    def _process_command(self, command: str, value: str):
        ...
        if command == '$button':
            ...
            print(f'Button {index} state: {state}')
            if int(index) == 1:
                if int(state) == 1:
                    mouse.press('left')
                else:
            ...
            print(f'\033[1;34mInfo: {value}\033[0m')
        elif command == '$error':
    ...
    def _process_measurment(self):
        ...
        if time.time() - self.previous_print > 0.05:
            ...
            debug_line += f'\r{clear_line()}  magnetometer: {self.measurment.magnetometer[0]:+20.5f} {self.measurment.magnetometer[1]:+20.5f} {self.measurment.magnetometer[2]:+20.5f} \n'
            debug_line += f'\r{clear_line()}   temperature: {self.measurment.temperature:+20.5f} \n'
            debug_line += f'\r{clear_line()}          time: {self.measurment.time:20.5f}    delta: {self.measurment_delta.time:10.5f}\n'
            ...
            debug_line += f'\r{clear_line()}      rotation: {self.measurment.rotation[0]:+20.5f} {self.measurment.rotation[1]:+20.5f} {self.measurment.rotation[2]:+20.5f} \n'
    ...
    def _process_calibration(self):
        ...
        if len(self.calibration_measurments) < 20:
            return
