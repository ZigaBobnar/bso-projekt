...
class GyroMouseSerial:
    def __init__(self, serial_port: str, serial_baud=115200):
        ...
        self.current_measurment_variables = []
    ...
    def _process_data(self, data: str):
        ...
        self.current_measurment_variables.append([variable, value])
    ...
    def _process_measurment(self):
        ...
        self.measurment.rotation = [self.previous_measurment.rotation[i] + self.measurment.gyroscope[i] * self.measurment_delta.time for i in range(3)]
        ...
        if time.time() - self.previous_print > 0.05:
            ...
            debug_line = f'\r\033[F\033[F\033[F\033[F\033[F\033[F\033[F\033[F\033[K'
            ...
            debug_line += f'vel: {self.measurment.velocity[0]:+10.5f} {self.measurment.velocity[1]:+10.5f} {self.measurment.velocity[2]:+10.5f} \n\033[K'
            debug_line += f'pos: {self.measurment.position[0]:+10.5f} {self.measurment.position[1]:+10.5f} {self.measurment.position[2]:+10.5f} \n\033[K'
            debug_line += f'rot: {self.measurment.rotation[0]:+10.5f} {self.measurment.rotation[1]:+10.5f} {self.measurment.rotation[2]:+10.5f} \n\033[K'
            # variables = ";".join([v[0] + "=" + v[1] for v in self.current_measurment_variables]).replace('$', '').replace('update=done', '')
            variables = ''
            debug_line += f'{variables}\n'
    ...
    def _calibrate(self):
        ...
        rotation[0] = math.atan2(avg_accel[1], avg_accel[2]) * 180 / math.pi
        rotation[1] = math.atan2(avg_accel[0], avg_accel[2]) * 180 / math.pi
        rotation[2] = math.atan2(avg_accel[1], avg_accel[0]) * 180 / math.pi
        # rotation[0] = math.atan2(avg_accel[1], avg_accel[2]) * 180 / math.pi
        # rotation[1] = math.atan2(avg_accel[0], avg_accel[2]) * 180 / math.pi
        # rotation[2] = math.atan2(avg_accel[1], avg_accel[0]) * 180 / math.pi
